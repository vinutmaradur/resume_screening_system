...
# 🔥 IMPORTANT: Call BEFORE pyresparser is used
ensure_nltk_data()
...
def extract_faces(image_path, model=None, class_labels=None):
    ...
    for (x, y, w, h) in faces:
        ...
        # No ML prediction (removed TensorFlow)
...
def create_connection():
    return mysql.connector.connect(
        host=st.secrets["mysql"]["host"],      # Change if needed (e.g., remote server)
        user=st.secrets["mysql"]["user"],  # Replace with your MySQL username
        password=st.secrets["mysql"]["password"],  # Replace with your MySQL password
        database=st.secrets["mysql"]["database"]   # Ensure you have created this database
    )
    ...
    if choice == 'User':
        ...
        if pdf_file is not None:
            ...
            for img_path in extracted_images:
                face, label = extract_faces(img_path)
                if face is not None:
